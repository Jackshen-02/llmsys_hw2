    ...
    int fn_id)
{
  ...
  int out_idx = blockIdx.x * blockDim.x + threadIdx.x;  // 1
  if (out_idx < out_size) {
    to_index(out_idx, out_shape, out_index, shape_size);  // 2
    ...
    int fn_id)
{
  ...
  int out_idx = blockIdx.x * blockDim.x + threadIdx.x;  // 1
  if (out_idx < out_size) {
    to_index(out_idx, out_shape, out_index, out_shape_size);  // 2
    ...
    int fn_id)
{
  ...
  if (out_idx < out_size) {
    ...
    float reduce_val = reduce_value;  // 3
